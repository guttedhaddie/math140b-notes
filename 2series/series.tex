...
\footnotetext{Recall that every sequence $(s_n)$ has subsequences tending to each of 
\[\limsup s_n=\lim_{N\to\infty}\sup\{x_n:n>N\}\quad\text{and}\quad \liminf s_n=\lim_{N\to\infty}\inf\{x_n:n>N\}\]
...
\begin{enumerate}
  \item $s_n-rs_n=1+r+\cdots+r^n-(r+\cdots +r^n+r^{n+1})=1-r^{n+1}\implies s_n=\frac{1-r^{n+1}}{1-r}$.
  \item $b_n=\frac 1n-\frac 1{n+1}\implies s_n=\left(1-\frac 12\right)+\left(\frac 12-\frac 13\right)+\cdots+\left(\frac 1n-\frac 1{n+1}\right)=1-\frac 1{n+1}$.
  \item Use the comparison or integral tests. Alternatively: For each $n\ge 2$, we have $\frac 1{n^2}<\frac 1{n(n-1)}$. By part 2,
  \[s_n=\sum_{k=1}^n\frac 1{k^2}<1+\sum_{k=1}^n\frac 1{k(k-1)}\le 2\]
  Since $(s_n)$ is a monotone up sequence, bounded above by 2, we conclude that $\sum\frac 1{n^2}$ is convergent.
  \item Use the integral test. Alternatively, observe that
  \[s_{2^{n+1}}-s_{2^n}=\sum_{k=2^n-1}^{2^{n+1}}\frac 1k\ge\frac{2^n}{2^{n+1}}=\frac 12 \implies s_{2^n}\ge\frac n2\xrightarrow[n\to\infty]{}\infty\]
  Since $s_n=\sum\limits_{k=1}^n\frac 1k$ defines an increasing sequence we conclude that $s_n\to\infty$.
  \item Use the alternating series test, or explicitly check that both the even and odd partial sums $(s_{2n})$ and $(s_{2n+1})$ are convergent (monotone and bounded) to the same limit.
\end{enumerate}
Root Test: $\beta<1\implies\exists \epsilon>0$ such that $\nm{b_n}^{1/n}\le 1-\epsilon$ (for large $n$) $\implies\sum\nm{b_n}$ converges by comparison with the convergent geometric series $\sum(1-\epsilon)^n$.\smallbreak
$\beta>1\implies$ a subsequence of $(\nm{b_n}^{1/n})$ converges to $\beta>1$, whence $b_n\nrightarrow 0\implies \sum b_n$ diverges ($n$\th-term test).}
...
% \begin{proof}
% If $\beta<1$, then, for large $n$, $\exists \epsilon>0$ such that $\nm{b_n}^{1/n}\le 1-\epsilon$. But then $\sum b_n$ converges by comparison with the convergent geometric series $\sum (1-\epsilon)^n$.\\
% If $\beta>1$, then a subsequence of $(\nm{b_n}^{1/n})$ converges to $\beta>1$, whence $(b_n)$ does not converge to zero. By the $n^\text{th}$-term/divergence test, the series $\sum b_n$ diverges.
% \end{proof}
...
As the example shows, we cannot take limits inside an infinite sum; understanding when we can do this is one of our primary goals. 

\goodbreak

% While understanding when a  goal of this course is not to evaluate sums of power series, or even to establish their interval of convergence. We have more general questions:
% \begin{enumerate}
%   \item When, and on what domain, does a given function $f$ equal a power series?
%   \item If a function equals a power series $f(x)=\sum_{n=0}^\infty a_n(x-c)^n$, how accurate is a given polynomial estimate $\sum_{k=0}^n a_k(x-c)^k$?
%   \item Are power series continuous, differentiable, integrable, and can we compute limits, derivatives and integrals in the obvious, term-by-term, manner?
% \end{enumerate}
% For instance, in the example on \pageref{ex:bob} we see that we can evaluate limits by interchanging the order of limit and infinite sum
% \[\lim\limits_{x\to a}\sum_{n=0}^\infty \frac{(-1)^n}{2^n}(x-4)^n =\lim\limits_{x\to a}\frac 2{x-2}=\frac 2{a-2} =\sum_{n=0}^\infty \frac{(-1)^n}{2^n}(a-4)^n =\sum_{n=0}^\infty \lim\limits_{x\to a}\frac{(-1)^n}{2^n}(x-4)^n\]
% but \emph{only} when $2<a<6$; if we try to do this as $x\to 6^-$ (at an \emph{endpoint} of the interval of convergence), the result is false. One of our purposes is to show that this interchangability, and the caveat regarding endpoints, is true in general, for limits, derivatives and integrals.
...
Given a series \smash[b]{$\sum\limits_{n=0}^\infty a_n(x-c)^n$,} define\footnotemark
\[R=\frac 1{\limsup\nm{a_n}^{1/n}}\]
...
\end{array}\]
% \begin{itemize}
%   \item[]$R=\infty$; the series converges absolutely for all $x\in\R$.
%   \item $R=0$; the series converges only when $x=c$.
%   \item $R\in\R^+$; the series converges absolutely when $\nm{x-c}<R$ and diverges when $\nm{x-c}>R$.
% \end{itemize}
\end{thm}

\footnotetext{Since $\nm{a_n}\ge 0$, we here adopt the convention that $\frac 10=\infty$ and $\frac 1\infty=0$. With similar caveats, it is also reasonable to write $R=\liminf\nm{a_n}^{-1/n}$.}
...
By applying Corollary \ref{cor:ratiotest}, we obtain a more user-friendly result.
...
\begin{enumerate}\setcounter{enumi}{1}
	\item[]\begin{enumerate}
	  \item \makebox[120pt][l]{$\displaystyle\sum\frac{(-1)^n}{n^24^n}x^n$\hfill(b)}
		\space \makebox[140pt][l]{$\displaystyle\sum\frac{(n+1)^2}{n^3}(x-3)^n$\hfill(c)}
		\space $\displaystyle\sum\sqrt nx^n$
		\item[(d)] \makebox[120pt][l]{$\displaystyle\sum\frac{1}{n^{\sqrt n}}(x+7)^n$\hfill(e)}
		\space \makebox[140pt][l]{$\displaystyle\sum (x-\pi)^{n!}$\hfill(f)}
...
There turn out to be several good notions of convergence for sequences of functions; the simplest it where, for each $x$, $(f_n(x))$ is treated as a separate sequence of real numbers.\goodbreak
...
\begin{exercises}
\exstart\phantomsection\label{exs:uniform1} For each sequence of functions defined on $[0,\infty)$:\vspace{-1pt}
